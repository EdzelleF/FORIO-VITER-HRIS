    ...
    <>
      <div className="print:hidden">
        <nav
          ...
          } h-dvh duration-200 ease-in fixed z-40  overflow-y-auto w-[14rem] print:hidden py-3 uppercase pt-[15px]`}
          ref={scrollRef}
              ...
              {navigationList?.map((item, key) => {
                ...
                return (
                  <li key={key} className={`h-fit flex items-center gap-2 ${item.subNavList && "flex-col gap-0.5!"}`}>
                    {item.subNavList ? (
                      <NavigationAccordions subNavList={item.subNavList} 
                      item={item}
                      />
                    )
                    : (
                    <Link to={item.path} className="w-full px-4 py-1 hover:bg-gray-50/10">
